...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] =  {
      ...
      fs.foldRight(successful.future){
        (fs, acc) => for {x <- fs; xs <- acc} yield x :: xs
      }
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      for {
        _fs <- fs
      } yield(_fs onComplete {p.tryComplete(_)})
      p.future
    }
    ...
    def delay(t: Duration): Future[Unit] = {
      future { blocking(Thread.sleep(t.toMillis)); Unit }
    }
  ...
  object CancellationTokenSource {
     ...
     */
    //println("CancellationTokenSource")
    
    def apply(): CancellationTokenSource = new CancellationTokenSource {
   
      //val p = Promise[Boolean]() ===> WHY do we have to use promises?? Hint says so... Maybe to eliminate the var
      var x = false;
      
      def cancellationToken: CancellationToken = new CancellationToken {
        //println("in cancellationToken")
        def isCancelled = x
      }
      
      def unsubscribe {
        //println("in unsubscribe")
        x = true
      }
