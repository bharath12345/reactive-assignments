...
object SchedulerEx {
  ...
  val SwingEventThreadScheduler: Scheduler = new Scheduler {
    ...
    def asJavaScheduler = new rx.Scheduler {
      ...
      def schedule[T](state: T, 
          func: rx.util.functions.Func2[_ >: rx.Scheduler, _ >: T, _ <: rx.Subscription], 
          delay: Long, units: java.util.concurrent.TimeUnit): rx.Subscription = {
      ...
      def schedule[T](state: T, 
          func: rx.util.functions.Func2[_ >: rx.Scheduler, _ >: T, 
          _ <: rx.Subscription]): rx.Subscription = {
